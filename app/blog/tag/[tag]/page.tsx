...
import { Calendar, User, TagIcon, ArrowLeft } from "lucide-react"
  ...
  useEffect(() => {
    const fetchPostsByTag = async () => {
      try {
        ...
        console.log(`Chargement des articles avec le tag: ${decodedTag}`)

        try {
          // Appel API pour récupérer les articles par tag
          const tagPosts = await API.blog.getPostsByTag(decodedTag)

          if (tagPosts && tagPosts.length > 0) {
            console.log(`${tagPosts.length} articles trouvés pour le tag: ${decodedTag}`)
            setPosts(tagPosts)
          } else {
            console.log(`Aucun article trouvé pour le tag: ${decodedTag}`)
            setPosts([])
          }
        } catch (apiErr) {
          console.error(`Erreur lors de la récupération des articles pour le tag ${decodedTag}:`, apiErr)
          setError(`Impossible de charger les articles pour le tag "${decodedTag}". Veuillez réessayer plus tard.`)
          setPosts([])
        }
      } catch (err: any) {
        ...
        <div className="container mx-auto">
          ...
          <nav className="mb-6 text-sm">
            <ol className="flex items-center space-x-2">
              ...
              <li className="text-light-brown/70">/</li>
              <li>
                <span className="text-light-brown font-medium">Tag: {decodedTag}</span>
              </li>
            </ol>
          </nav>
          ...
          <div className="mb-10 text-center">
            <h1 className="font-script text-3xl md:text-4xl text-light-brown mb-4">
              Articles avec le tag "{decodedTag}"
            </h1>
            <p className="text-light-brown/80 max-w-2xl mx-auto">
              Découvrez tous nos articles liés à ce sujet et approfondissez vos connaissances.
            </p>
          </div>
          ...
          {isLoading ? (
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                        {post.tags &&
                          (Array.isArray(post.tags)
                            ...
                            <span
                              key={tag}
                              className={`inline-flex items-center px-2 py-1 rounded-full text-xs ${tag.toLowerCase() === decodedTag.toLowerCase() ? "bg-soft-green/20 text-soft-green" : "bg-beige/50 text-light-brown"}`}
                            >
            ...
            <div className="text-center py-12 bg-white rounded-lg shadow-md">
              <p className="text-light-brown text-lg mb-4">Aucun article trouvé pour le tag "{decodedTag}"</p>
              <Button className="bg-soft-green hover:bg-soft-green/90 text-white" onClick={() => router.push("/blog")}>
                <ArrowLeft className="h-4 w-4 mr-2" />
                Retour au blog
              </Button>
