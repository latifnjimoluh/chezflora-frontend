  ...
  useEffect(() => {
    const fetchPostsByCategory = async () => {
      try {
        ...
        console.log(`Chargement des articles de la catégorie: ${decodedCategory}`)

        try {
          // Appel API pour récupérer les articles par catégorie
          const categoryPosts = await API.blog.getPostsByCategory(decodedCategory)

          if (categoryPosts && categoryPosts.length > 0) {
            console.log(`${categoryPosts.length} articles trouvés pour la catégorie: ${decodedCategory}`)
            setPosts(categoryPosts)
          } else {
            console.log(`Aucun article trouvé pour la catégorie: ${decodedCategory}`)
            setPosts([])
          }
        } catch (apiErr) {
          console.error(`Erreur lors de la récupération des articles pour la catégorie ${decodedCategory}:`, apiErr)
          setError(
            `Impossible de charger les articles pour la catégorie "${decodedCategory}". Veuillez réessayer plus tard.`,
          )
          setPosts([])
        }
      } catch (err: any) {
        ...
        <div className="container mx-auto">
          ...
          <nav className="mb-6 text-sm">
            <ol className="flex items-center space-x-2">
              ...
              <li className="text-light-brown/70">/</li>
              <li>
                <span className="text-light-brown font-medium">Catégorie: {decodedCategory}</span>
              </li>
            </ol>
          </nav>
          ...
          <div className="mb-10 text-center">
            <h1 className="font-script text-3xl md:text-4xl text-light-brown mb-4">
              Articles de la catégorie "{decodedCategory}"
            </h1>
            <p className="text-light-brown/80 max-w-2xl mx-auto">
              Découvrez tous nos articles dans cette catégorie et approfondissez vos connaissances.
            </p>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12 bg-white rounded-lg shadow-md">
              <p className="text-light-brown text-lg mb-4">
                Aucun article trouvé pour la catégorie "{decodedCategory}"
              </p>
              <Button className="bg-soft-green hover:bg-soft-green/90 text-white" onClick={() => router.push("/blog")}>
                <ArrowLeft className="h-4 w-4 mr-2" />
                Retour au blog
